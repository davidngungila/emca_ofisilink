...
async function triggerBackup() {
    ...
    try {
        ...
        if(data.success) {
            ...
            result.innerHTML = '<i class="bx bx-check-circle"></i> <strong>Backup Completed Successfully!</strong><br><small>All System Administrators have been notified via SMS and Email with attachment.</small><br>' + (data.download_url ? '<a href="' + data.download_url + '" class="btn btn-sm btn-primary mt-2" download><i class="bx bx-download"></i> Download Backup File</a>' : '<small class="text-muted">Download link will be available in the backups list below.</small>');

            Swal.fire({
                title: 'Backup Completed!',
                html: '<p>Database backup has been created successfully.</p><p><strong>All System Administrators have been notified via SMS and Email with attachment.</strong></p>' + (data.download_url ? '<p><a href="' + data.download_url + '" class="btn btn-primary mt-2" download>Download Backup</a></p>' : '<p class="text-warning">Download link will be available in the backups list.</p>'),
                icon: 'success',
